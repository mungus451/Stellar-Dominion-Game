...
try {
    ...
    if (!isset($link) || !$link) {
        error_log("advisor_poll.php: Database connection not available");
    ...
    if (!$user) {
        error_log("advisor_poll.php: User data not found for ID: " . $user_id);
    ...
    error_log("advisor_poll.php FATAL ERROR: " . $e->getMessage() . " in " . $e->getFile() . " on line " . $e->getLine());
    
    http_response_code(500);
    echo json_encode([
        'error' => 'Internal server error',
        'debug' => isset($_ENV['AWS_LAMBDA_FUNCTION_NAME']) ? null : $e->getMessage()
    ]);
    exit;
}
