...
{
    private const CREDITS_TO_GEMS_NUM = 93;   // 100 credits -> 93 gems (7% house)
    ...
    public function convertCreditsToGems(PDO $pdo, int $userId, int $creditsInput): array
    {
        ...
        try {
            ...
            // Gems = floor(0.93 * credits); fee (credits) = credits - gems
            $gemsReceived = intdiv($creditsInput * self::CREDITS_TO_GEMS_NUM, self::CREDITS_TO_GEMS_DEN);
            $feeCredits   = $creditsInput - $gemsReceived;

            $pdo->prepare("UPDATE users SET credits = credits - ?, gemstones = gemstones + ? WHERE id=?")
                ->execute([$creditsInput, $gemsReceived, $userId]);

            if ($feeCredits > 0) $this->bumpHouse($pdo, 'credits_collected', $feeCredits);
            ...
            if ($this->tableHasColumn($pdo, 'black_market_conversion_logs', 'house_fee_credits')) {
                $pdo->prepare("INSERT INTO black_market_conversion_logs
                    ...
                    ->execute([$userId, $creditsInput, $gemsReceived, $feeCredits]);
            }

            $pdo->commit();
            return ['credits_delta'=>-$creditsInput,'gemstones_delta'=>$gemsReceived,'house_fee_credits'=>$feeCredits];
        ...
        try {
            ...
            if ($this->tableHasColumn($pdo, 'black_market_conversion_logs', 'house_fee_credits')) {
                $pdo->prepare("INSERT INTO black_market_conversion_logs
                    ...
                    ->execute([$userId, $gemsInput, $creditsReceived, $feeCredits]);
            }

            $pdo->commit();
            return ['credits_delta'=>$creditsReceived,'gemstones_delta'=>-$gemsInput,'house_fee_credits'=>$feeCredits];
        } catch (\Throwable $e) { $pdo->rollBack(); throw $e; }
    }
    ...
    public function startMatch(PDO $pdo, int $userId, int $betGemstones = 0): array
    {
        ...
        try {
            ...
            return [
                ...
                'house_gemstones_delta'  =>  $needed, // show House growth in UI
            ];
        } catch (\Throwable $e) { $pdo->rollBack(); throw $e; }
    }
    ...
    private function getActiveRound(PDO $pdo, int $matchId, bool $forUpdate=false): array
    {
        ...
        $st = $pdo->prepare($sql); $st->execute([$matchId]); $round = $st->fetch(); if(!$round) throw new RuntimeException('no rounds yet'); return $round;
    }
