 ...
 * Includes logic for unit destruction and foundation damage.
 */
...
try {
    ...
    for ($i = 0; $i < $attacker['soldiers'] * $attack_turns; $i++) { $attacker_base_power += rand(8, 12); }
    $attacker_total_power = floor(($attacker_base_power * $strength_bonus) * $offense_upgrade_multiplier);
    $attacker_damage = floor($attacker_total_power * (rand(90, 110) / 100));
    ...
    for ($i = 0; $i < $defender['guards']; $i++) { $defender_base_power += rand(8, 12); }
    $defender_total_power = floor(($defender_base_power * $constitution_bonus) * $defense_upgrade_multiplier);
    $defender_damage = floor($defender_total_power * (rand(90, 110) / 100));
    ...
    if ($outcome === 'victory') {
        ...
        $guards_lost = min($defender['guards'], $guards_lost);
    }
    ...
    $attacker_level_mod = max(0.1, 1 + ($level_diff_attacker * ($level_diff_attacker > 0 ? 0.07 : 0.1)));
    ...
    if ($outcome === 'victory') {
        ...
        mysqli_query($link, "UPDATE users SET credits = credits - {$credits_stolen}, experience = experience + {$defender_xp_gained}, guards = guards - {$guards_lost}, fortification_hitpoints = GREATEST(0, fortification_hitpoints - {$structure_damage}) WHERE id = {$defender_id}");
    } else { // Defeat
