...
// CSRF helper (must define generate_csrf_token())
...
<head>
    ...
    <title>Starlight Dominion - Reset Password</title>
...
<div class="min-h-screen bg-cover bg-center bg-fixed" style="background-image:url('/assets/img/backgroundMain.avif');">
    <div class="container mx-auto p-4 md:p-8 flex items-center justify-center min-h-screen">
        <div class="content-box rounded-lg p-8 max-w-md w-full">
            <h1 class="font-title text-3xl text-cyan-400 border-b border-gray-600 pb-2 mb-4 text-center">
                Reset Your Password
            </h1>
            ...
            <!-- IMPORTANT: post to routed endpoint (/auth), not /src/... -->
            <form action="/auth" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="password" id="verify_password" name="verify_password" required minlength="8"
                           class="w-full bg-gray-900 border border-gray-600 rounded-md p-2 mt-1">
                </div>

                <div class="text-center">
                    <button type="submit" class="bg-cyan-600 hover:bg-cyan-700 text-white font-bold py-3 px-8 rounded-lg">
                        Set New Password
                    </button>
                </div>
            </form>
