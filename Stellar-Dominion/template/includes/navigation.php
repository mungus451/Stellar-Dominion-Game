...
// We can safely assume $user_id is set by header.php
// We can safely assume $link (mysqli) is set by config.php or header.php
global $link;
$user_id = (int)($_SESSION['id'] ?? 0);

if ($user_id <= 0) {
    // Should not happen if we're logged in, but as a safeguard...
    return; 
}

// Fetch minimal nav data
$stmt = $link->prepare(
    "SELECT 
        u.character_name, u.credits, u.gemstones, u.avatar_path,
...
$character_name = htmlspecialchars($nav_user['character_name'] ?? 'Player');
$avatar_path = htmlspecialchars($nav_user['avatar_path'] ?? '/assets/img/human.avif');
$alliance_id = (int)($nav_user['alliance_id'] ?? 0);
$alliance_name = htmlspecialchars($nav_user['alliance_name'] ?? 'No Alliance');
    ...
    <div class="flex justify-center flex-wrap items-center gap-x-4 gap-y-1 md:gap-x-6 bg-gray-900 p-2 rounded-t-md">
        ...
        <div class="flex items-center" title="Credits">
            ...
            <span id="credits-display" class="font-bold text-white">
                <?php echo number_format($nav_user['credits'] ?? 0); ?>
            </span>
        </div>
        ...
        <div class="flex items-center" title="Gemstones">
            ...
            <span id="gemstones-display" class="font-bold text-white">
                <?php echo number_format($nav_user['gemstones'] ?? 0); ?>
            </span>
        </div>
